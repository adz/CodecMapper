...
- The remaining BenchmarkDotNet block is still in the autogenerated child project on this environment and SDK combination.
- Specifically, the child build exits with code `1` during `_GetProjectReferenceTargetFrameworkProperties` against `CodecMapper.Benchmarks.fsproj`, with `Build FAILED` but `0 Error(s)` surfaced by MSBuild.
