...
- **BenchmarkDotNet is still partially blocked on this SDK.** The top-level benchmark app is runnable again, but the autogenerated child project still fails in `_GetProjectReferenceTargetFrameworkProperties` with `0 Error(s)`. Use `src/CodecMapper.Benchmarks.Runner` for published numbers unless that underlying tooling issue is resolved.
- **The local legacy comparison harness is intentionally isolated.** `src/CodecMapper.CompareRunner` compares current `CodecMapper` against the archived experimental clone under `benchmarks/CodecMapper/`, and that local clone is expected to emit `CodecMapper.CPExperiment.dll` to avoid colliding with the current assembly name.
