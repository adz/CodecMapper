...
- **KeyValue and YAML are config-oriented projections, not general dynamic containers.** `KeyValue` is for flattened record/scalar shapes, and YAML currently supports only a small handwritten subset over mappings, sequences, scalars, and `null`. Keep broader normalization, anchors/tags, and full YAML feature parity out of scope unless the repo deliberately chooses to expand them.
...
- **Compatibility sentinels now share one library-level runner.** Keep the common coverage in `tests/CodecMapper.CompatibilitySentinel/`, and let the AOT and Fable apps stay as thin shells so new compatibility cases only need to be added once.
...
- **The Fable compatibility story now has two pinned transpilation lanes.** `scripts/check-fable-compat.sh` keeps the stable `Fable 4` baseline, and `scripts/check-fable5-compat.sh` checks `Fable 5.0.0-rc.2` with its newer CLI shape. Keep both lanes green when touching generic or type-driven code.
...
- **Do not use `System.Enum.ToObject` or `System.Convert.ChangeType` in the core portable path.** Fable rejects both APIs. When adding enum support, keep the .NET path behind `#if !FABLE_COMPILER` and use a Fable-safe boxed-number path instead.
