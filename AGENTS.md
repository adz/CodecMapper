...
- **Common built-in schemas are now broader, but still intentional.** Auto-resolution currently includes `int16`, `byte`, `sbyte`, `uint16`, `char`, `Guid`, `DateTime`, `DateTimeOffset`, and `TimeSpan` in addition to the original primitives, lists, arrays, and `Schema.map`.
