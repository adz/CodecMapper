...
module Core =
    ...
    type ResizableBuffer =
        ...
        static member Create(initialCapacity: int) =
            { InternalData = Array.zeroCreate initialCapacity; InternalCount = 0 }
        ...
        interface IByteWriter with
            ...
            member x.WriteInt(value: int) =
                if value = 0 then (x :> IByteWriter).WriteByte(48uy)
                else
                    let mutable v = value
                    if v < 0 then
...
module Schema =
    ...
    let inline list (inner: Schema<'T>) : Schema<'T list> =
        create (List(inner :> ISchema))

    let inline array (inner: Schema<'T>) : Schema<'T[]> =
        create (Array(inner :> ISchema))

    let rec resolveSchema (t: System.Type) : ISchema =
        if t = typeof<int> then int :> ISchema
        elif t = typeof<string> then string :> ISchema
        elif t.IsGenericType && t.GetGenericTypeDefinition() = typeof<list<_>>.GetGenericTypeDefinition() then
    ...
    let inline record<'T, 'Anon> (mapping: 'T -> 'Anon) : Schema<'T> =
        ...
        let fields = FSharpType.GetRecordFields(anonType, allowAccessToPrivateRepresentation=true)
        let schemaFields = 
            fields 
            |> Array.map (fun f -> 
                { Name = f.Name
        ...
        let targetFields = FSharpType.GetRecordFields(targetType, allowAccessToPrivateRepresentation=true)
        #if !FABLE_COMPILER
        let ctor = FSharpValue.PreComputeRecordConstructor(targetType, allowAccessToPrivateRepresentation=true)
        #endif
        let build (args: obj[]) =
            let sortedArgs = 
                targetFields 
                |> Array.map (fun tf ->
                    let idx = schemaFields |> Array.findIndex (fun sf -> sf.Name.Equals(tf.Name, System.StringComparison.OrdinalIgnoreCase))
                    args.[idx])
            #if !FABLE_COMPILER
            ctor sortedArgs
            #else
    ...
    let inline recordWith<'T, 'Anon> (mapping: 'T -> 'Anon) (fieldSchemas: Map<string, ISchema>) : Schema<'T> =
        ...
        let schemaFields = 
            fields 
            |> Array.map (fun f -> 
                let s = match fieldSchemas.TryFind f.Name with Some s -> s | None -> resolveSchema f.PropertyType
                { Name = f.Name
                  Type = f.PropertyType
                  GetValue = fun r -> f.GetValue(mapping (r :?> 'T))
                  Schema = s })
...
module Json =
    ...
    module internal Runtime =
        let inline skipWhitespace (src: JsonSource) =
            ...
            let data = src.Data
            while i < data.Length && (let b = data.[i] in b = 32uy || b = 10uy || b = 13uy || b = 9uy) do
                i <- i + 1
            ByteSource(data, i)

        let intDecoder : Decoder<int> = fun src ->
            let src = skipWhitespace src
            if src.Offset >= src.Data.Length then failwith "Unexpected end of input"
            let mutable i = src.Offset
            let mutable res = 0
        ...
        let rec skipValue (src: JsonSource) : JsonSource =
            let src = skipWhitespace src
            if src.Offset >= src.Data.Length then src
            else
                let data = src.Data
                ...
                | 123uy -> 
                    ...
                    while depth > 0 && i < data.Length do
                        if data.[i] = 123uy then depth <- depth + 1
                        elif data.[i] = 125uy then depth <- depth - 1
                        i <- i + 1
                    ByteSource(data, i)
                | 91uy -> 
                    let mutable depth = 1
                    let mutable i = src.Offset + 1
                    while depth > 0 && i < data.Length do
                        if data.[i] = 91uy then depth <- depth + 1
                        elif data.[i] = 93uy then depth <- depth - 1
                        i <- i + 1
                    ByteSource(data, i)
                | 34uy -> 
        ...
        let inline bytesEqual (a: byte[]) (b: byte[]) (offset: int) (len: int) =
            if a.Length <> len then false
            else
                let mutable i = 0
                let mutable equal = true
                while i < len && equal do
                    if a.[i] <> b.[offset + i] then equal <- false
                    else i <- i + 1
                equal
    ...
    let rec compileUntyped (schema: ISchema) : CompiledCodec =
        ...
        | Record(t, fields, ctor) ->
            let compiledFields = 
                ...
                |> Array.mapi (fun i f ->
                    ...
                    {| Name = f.Name; KeyBytes = keyBytes; Index = i; Codec = codec |})
            let encoder (writer: JsonWriter) (vObj: obj) =
                writer.WriteByte(123uy)
                let mutable first = true
                for f in compiledFields do
                    if not first then writer.WriteByte(44uy)
                    writer.WriteByte(34uy); writer.WriteString(f.Name); writer.WriteByte(34uy); writer.WriteByte(58uy)
            ...
            let decoder (src: JsonSource) =
                ...
                if src.Offset >= src.Data.Length || src.Data.[src.Offset] <> 123uy then failwith "Expected {"
                let data = src.Data
                ...
                while looping do
                    ...
                    if afterColon.Offset >= data.Length || data.[afterColon.Offset] <> 58uy then failwith "Expected :"
                ...
                let args = 
                    ...
                    |> Array.map (fun f ->
                        ...
                        if valSrc.Data = null then failwithf "Missing required key: %s" f.Name
        ...
        | List innerSchema ->
            ...
            let decoder (src: JsonSource) =
                ...
                if src.Offset >= src.Data.Length || src.Data.[src.Offset] <> 91uy then failwith "Expected ["
                src <- src.Advance(1)
                let mutable results = []
...
module Xml =
    ...
    let rec compileUntyped (schema: ISchema) : CompiledCodec =
        ...
        | Primitive t when t = typeof<int> ->
            { Encode = (fun w tag v -> 
                w.WriteByte(60uy); w.WriteString(tag); w.WriteByte(62uy)
                w.WriteInt(unbox v)
                w.WriteByte(60uy); w.WriteByte(47uy); w.WriteString(tag); w.WriteByte(62uy))
              Decode = (fun src tag -> 
                let data = src.Data
                let mutable i = src.Offset
                if data.[i] <> 60uy then failwith "Expected <"
                i <- i + tag.Length + 2
                let start = i
                while data.[i] <> 60uy do i <- i + 1
        ...
        | Record(t, fields, ctor) ->
            let compiledFields = 
                fields |> Array.map (fun f -> {| Name = f.Name; Codec = compileUntyped f.Schema; GetValue = f.GetValue |})
            { Encode = (fun w tag vObj ->
                w.WriteByte(60uy); w.WriteString(tag); w.WriteByte(62uy)
                for f in compiledFields do
              ...
              Decode = (fun src tag ->
                ...
                if current.Data.[current.Offset] <> 60uy then failwith "Expected <"
