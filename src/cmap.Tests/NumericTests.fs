...
let ``Extended numeric decoders reject out-of-range values`` () =
    expectFailure "int64 value out of range" (fun () -> Json.deserialize (Json.compile Schema.int64) "9223372036854775808")
    expectFailure "uint32 value out of range" (fun () -> Json.deserialize (Json.compile Schema.uint32) "4294967296")
    expectFailure "uint64 value out of range" (fun () -> Json.deserialize (Json.compile Schema.uint64) "18446744073709551616")
